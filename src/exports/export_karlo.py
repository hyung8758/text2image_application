...
def export_karlo_from_source():
    ...
    tok, mask = tok.to(device="cuda:1"), mask.to(device="cuda:1")
    ...
    txt_faet=torch.randn(2,768).to(device="cuda:1")
    txt_feat_seq=torch.randn(2,77,768).to(device="cuda:1")
    mask=torch.ones(2,77).bool().to(device="cuda:1")
    prior_cf_scales_batch=torch.tensor([0.4]*1, device="cuda:1")
...
if __name__ == "__main__":
    import sys
    usageMessage = f"""
    Usage: {sys.argv[0]} [export type: source or library]
    """
    if len(sys.argv) == 2:
        if sys.argv[1] == "source":
            ...
            raise RuntimeError("Unknown arguments: {}".format(sys.argv[1]))
    else:
        print(usageMessage)